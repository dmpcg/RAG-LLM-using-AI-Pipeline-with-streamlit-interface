...
def sample_covenant_package():
    ...
    return CovenantPackage(
        ...
        financial_covenants={
            "min_current_ratio": {"threshold": 1.25, "frequency": "quarterly", "description": "Minimum current ratio"},
            "max_debt_to_ebitda": {
                "threshold": 3.5,
                "frequency": "quarterly",
...
class TestStoreCreditAssessment:
    def test_returns_assessment_id_on_success(self, store, mock_driver, sample_scorecard, sample_debt_capacity):
        _, session = mock_driver
        assessment_id = store.store_credit_assessment("Acme Corp", sample_scorecard, sample_debt_capacity)
    ...
    def test_assessment_batch_contains_correct_fields(self, store, mock_driver, sample_scorecard, sample_debt_capacity):
        _, session = mock_driver
        store.store_credit_assessment("Acme Corp", sample_scorecard, sample_debt_capacity)
    ...
    def test_assessment_id_is_deterministic(self, store, mock_driver, sample_scorecard, sample_debt_capacity):
        _, session = mock_driver
    ...
    def test_different_company_produces_different_id(self, store, mock_driver, sample_scorecard, sample_debt_capacity):
        _, session = mock_driver
        id1 = store.store_credit_assessment("Acme Corp", sample_scorecard, sample_debt_capacity)
    ...
    def test_handles_none_debt_capacity_fields(self, store, mock_driver, sample_scorecard):
        ...
        assert assessment_id is not None
        batch_arg = session.run.call_args_list[1][1].get("batch") or session.run.call_args_list[1][0][1]
        row = batch_arg[0]
        assert row["max_additional_debt"] is None
        assert row["current_leverage"] is None

    def test_returns_none_on_neo4j_failure(self, store, mock_driver, sample_scorecard, sample_debt_capacity):
    ...
    def test_uses_unwind_batch_query(self, store, mock_driver, sample_scorecard, sample_debt_capacity):
...
class TestStoreCovenantPackage:
    def test_returns_package_id_on_success(self, store, mock_driver, sample_covenant_package):
        _, session = mock_driver
    ...
    def test_creates_exactly_one_unwind_batch_call(self, store, mock_driver, sample_covenant_package):
        _, session = mock_driver
        assessment_id = "b" * 64
    ...
    def test_batch_contains_correct_fields(self, store, mock_driver, sample_covenant_package):
        _, session = mock_driver
        assessment_id = "c" * 64
        store.store_covenant_package(assessment_id, sample_covenant_package)
    ...
    def test_package_id_is_deterministic(self, store, mock_driver, sample_covenant_package):
        _, session = mock_driver
        assessment_id = "d" * 64
    ...
    def test_different_tier_produces_different_package_id(self, store, mock_driver):
        from underwriting import CovenantPackage
    ...
    def test_returns_none_on_neo4j_failure(self, store, mock_driver, sample_covenant_package):
    ...
    def test_uses_unwind_batch_query(self, store, mock_driver, sample_covenant_package):
    ...
    def test_handles_empty_covenant_package(self, store, mock_driver):
        ...
        assert package_id is not None
        batch_arg = session.run.call_args_list[0][1].get("batch") or session.run.call_args_list[0][0][1]
        row = batch_arg[0]
...
class TestCreditAssessmentCovenantChain:
    ...
    def test_full_chain_returns_both_ids(
        ...
        _, session = mock_driver
        assessment_id = store.store_credit_assessment("ChainCo", sample_scorecard, sample_debt_capacity)
        assert assessment_id is not None
    ...
    def test_covenant_batch_links_to_assessment_id(
        ...
        _, session = mock_driver
        assessment_id = store.store_credit_assessment("LinkCo", sample_scorecard, sample_debt_capacity)
        store.store_covenant_package(assessment_id, sample_covenant_package)
    ...
    def test_assessment_failure_does_not_prevent_package_attempt(
        ...
        with pytest.raises(Neo4jTransientError):
            store.store_credit_assessment("FailCo", sample_scorecard, sample_debt_capacity)
