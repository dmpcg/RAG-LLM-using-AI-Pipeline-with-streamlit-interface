...
def create_code_block(code_text, styles):
    ...
    code_para = Paragraph(f'<font face="Courier" color="#E5E7EB">{code_text}</font>', styles["CodeBlock"])
        ...
        if "**" in tech:
            ...
            if len(parts) >= 3:
                ...
                cell_content = Paragraph(
                    f'<b><font color="#1E40AF">{name}</font></b><br/><font size="9" color="#4B5563">{desc}</font>',
                    styles["CustomBody"],
                )
            else:
...
def build_pdf(output_path):
    ...
    # Title with emoji
    story.append(Paragraph("RAG-LLM Using AI Pipeline with<br/>Streamlit Interface", styles["MainTitle"]))

    # Subtitle/tagline
    story.append(
        Paragraph(
            "Retrieval-Augmented Generation powered by Claude Sonet 3.5 LLM and Pathway Framework", styles["Subtitle"]
        )
    )
    ...
    for item in toc_items:
        story.append(Paragraph(f'<font color="#1E40AF">{item}</font>', styles["TOCEntry"]))
    ...
    story.append(
        Paragraph("This project employs a variety of technologies to achieve its goals:", styles["CustomBody"])
    )
    ...
    story.append(Paragraph("To set up the project locally, follow these steps:", styles["CustomBody"]))

    # Step 1
    story.append(Paragraph("Step 1: Clone the Repository", styles["StepNumber"]))
    story.append(
        create_code_block(
            "git clone https://github.com/Dono1901/RAG-LLM-using-AI-Pipeline-with-streamlit-interface.git", styles
        )
    )

    # Step 2
    story.append(Paragraph("Step 2: Navigate to the Project Directory", styles["StepNumber"]))
    story.append(create_code_block("cd RAG-LLM-using-AI-Pipeline-with-streamlit-interface", styles))

    # Step 3
    story.append(Paragraph("Step 3: Install Dependencies", styles["StepNumber"]))
    story.append(Paragraph("Make sure you have Python 3.8 or higher installed. Then, run:", styles["CustomBody"]))
    ...
    story.append(Paragraph("After setting up the application, you can start using it:", styles["CustomBody"]))
    ...
    for i, step in enumerate(usage_steps, 1):
        story.append(Paragraph(f'<font color="#0D9488"><b>{i}.</b></font>  {step}', styles["FeatureItem"]))
    ...
    story.append(Paragraph('<font color="#0D9488"><b>1.</b></font>  <b>Fork the Repository</b>', styles["FeatureItem"]))

    story.append(
        Paragraph('<font color="#0D9488"><b>2.</b></font>  <b>Create a New Branch:</b>', styles["FeatureItem"])
    )
    story.append(create_code_block("git checkout -b feature/YourFeature", styles))

    story.append(Paragraph('<font color="#0D9488"><b>3.</b></font>  <b>Make Your Changes</b>', styles["FeatureItem"]))

    story.append(
        Paragraph('<font color="#0D9488"><b>4.</b></font>  <b>Commit Your Changes:</b>', styles["FeatureItem"])
    )
    story.append(create_code_block('git commit -m "Add some feature"', styles))

    story.append(Paragraph('<font color="#0D9488"><b>5.</b></font>  <b>Push to the Branch:</b>', styles["FeatureItem"]))
    story.append(create_code_block("git push origin feature/YourFeature", styles))

    story.append(Paragraph('<font color="#0D9488"><b>6.</b></font>  <b>Open a Pull Request</b>', styles["FeatureItem"]))
    ...
    story.append(Paragraph("For any questions or feedback, feel free to reach out:", styles["CustomBody"]))

    contact_info = ["<b>Email:</b> your.email@example.com", "<b>Twitter:</b> @yourhandle"]

    for info in contact_info:
        story.append(Paragraph(f'<font color="#0D9488">&#x2022;</font>  {info}', styles["FeatureItem"]))
    ...
    story.append(
        Paragraph(
            "Thank you for your interest in the RAG-LLM Using AI Pipeline with Streamlit Interface!", styles["Footer"]
        )
    )
